...
fn make_chat_response(content: &str) -> Value {
    json!({
        "id": format!("chatcmpl-{}", content.len()),
...
/// Mount 5 different responses (3x "42", 1x "7", 1x "99") and verify
/// self-consistency returns the majority answer "42".
#[tokio::test]
async fn compat_self_consistency_varied_content() {
    let base_url = start_test_server().await;
    let client = reqwest::Client::new();
    let backend = MockServer::start().await;

    // Serve each response exactly once in order. After all 5 are consumed
    // the test is done, so priority ordering ensures deterministic dispatch.
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(make_chat_response("The answer is 42")))
        .up_to_n_times(1)
        .with_priority(1)
        .mount(&backend)
        .await;
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(make_chat_response("I think it is 7")))
        .up_to_n_times(1)
        .with_priority(4)
        .mount(&backend)
        .await;
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(make_chat_response("Probably 99")))
        .up_to_n_times(1)
        .with_priority(5)
        .mount(&backend)
        .await;
    ...
    let selected_content = body["choices"][0]["message"]["content"]
        ...
        .unwrap();
    assert_eq!(
        selected_content, "The answer is 42",
        "majority vote should select 'The answer is 42' (3 of 5), got '{}'",
        selected_content
    );
}

/// Mount 3 different responses, mock ORM returns [0.3, 0.9, 0.5].
/// Verify the server returns the highest-scored response.
#[tokio::test]
async fn compat_best_of_n_varied_scores() {
    let base_url = start_test_server().await;
    let client = reqwest::Client::new();
    let backend = MockServer::start().await;
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(make_chat_response("low quality answer")))
        .up_to_n_times(1)
        .with_priority(1)
        .mount(&backend)
        .await;
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(make_chat_response("high quality answer")))
        .up_to_n_times(1)
        .with_priority(2)
        .mount(&backend)
        .await;
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(make_chat_response("medium quality answer")))
        .up_to_n_times(1)
        .with_priority(3)
        .mount(&backend)
        .await;

    // ORM mock: scores [0.3, 0.9, 0.5] for the 3 distinct responses
    let orm = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/score"))
        .respond_with(
    ...
    let selected_content = body["choices"][0]["message"]["content"]
        ...
        .unwrap();
    assert_eq!(
        selected_content, "high quality answer",
        "best-of-n should select highest-scored response (score 0.9), got '{}'",
        selected_content
    );
}
