...
impl BeamSearch {
    ...
    async fn forward_steps(
        ...
        for (prompt, steps) in prompts.iter().zip(steps_so_far.iter()) {
            ...
            if !steps.is_empty() {
                let partial = self.sg.post_process(steps, false);
            ...
            results.push((content, is_stopped));
        }
...
mod tests {
    ...
    fn test_contains_stop_token() {
        ...
        assert!(bs.contains_stop_token("some text END here"));
        assert!(!bs.contains_stop_token("no stop here"));
    }
}
