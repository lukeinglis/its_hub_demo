...
use super::{AlgorithmOutput, ProcessRewardModel, ScalingAlgorithm};
use crate::chat_messages::ChatMessages;
use crate::client::LmClient;
use crate::step_generation::StepGeneration;
...
impl ParticleGibbs {
    ...
    fn build_stop_string(&self) -> Option<String> {
        if let Some(st) = self.sg.step_token() {
            match st {
                crate::step_generation::StepToken::Single(tok) => return Some(tok.clone()),
                crate::step_generation::StepToken::Multiple(toks) => {
        ...
        if let Some(ref stop) = self.sg.stop_token {
            return Some(stop.clone());
        }
        None
    }

    fn contains_stop_token(&self, text: &str) -> bool {
        if let Some(ref stop) = self.sg.stop_token {
    ...
    async fn propagate(
        ...
        for &idx in &active_indices {
            let mut messages: Vec<ChatMessage> = vec![ChatMessage {
                role: "user".to_string(),
                content: Some(crate::types::Content::Text(prompt.to_string())),
                tool_calls: None,
                tool_call_id: None,
            }];

            if !particles[idx].steps.is_empty() {
                let partial = self.sg.post_process(&particles[idx].steps, false);
                messages.push(ChatMessage {
                    role: "assistant".to_string(),
                    content: Some(crate::types::Content::Text(partial)),
                    tool_calls: None,
                    tool_call_id: None,
                });
            }

            let temp = temperature.unwrap_or(self.sg.temperature);
            let response = client
                .chat_completion(&messages, Some(temp), max_tokens, stop_str.as_deref(), tools, tool_choice)
                .await
            ...
            sg_results.push((content, is_stopped));
        }
        ...
        for (j, &idx) in active_indices.iter().enumerate() {
            ...
            let scores = self.prm.score(&chat_messages, &steps).await?;
