...
/// Abstraction over LM backends so algorithms and server state work with
/// either `LmClient` (direct OpenAI/vLLM) or `LiteLLMClient` (multi-provider).
pub enum LmBackend {
    OpenAI(LmClient),
...
impl LmBackend {
    ...
    pub async fn chat_completion(
        ...
        match self {
            LmBackend::OpenAI(c) => {
                c.chat_completion(messages, temperature, max_tokens, stop, tools, tool_choice)
                    .await
            }
            LmBackend::LiteLLM(c) => {
                c.chat_completion(messages, temperature, max_tokens, stop, tools, tool_choice)
