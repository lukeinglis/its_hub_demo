...
/// Strips `suffix` from the end of `s` only if the result would be non-empty.
/// If removing `suffix` would leave an empty string, returns `s` unchanged.
...
mod tests {
    ...
    fn test_rstrip_iff_entire() {
        ...
        assert_eq!(rstrip_iff_entire("world", "world"), "world");
        ...
        assert_eq!(rstrip_iff_entire("END", "END"), "END");
    }
}
