...
The model dropdowns in step 4 are dynamically populated based on which providers were detected in step 2. Curated questions are fetched from the backend `/examples` endpoint.
...
The guided demo uses real captured API responses from `guided-demo-data.json` (generated by `scripts/capture_guided_scenarios.py`). A badge on the results page indicates whether captured or example data is being shown.
...
### Guided Demo — Recapturing scenarios

The guided demo uses pre-captured API responses from `frontend/guided-demo-data.json`. To recapture with fresh data:

```bash
cd demo_ui
python scripts/capture_guided_scenarios.py                    # Recapture all scenarios
python scripts/capture_guided_scenarios.py --scenario improve_frontier_self_consistency  # Single scenario
```

This calls the `/compare` endpoint for each scenario and saves the results. The backend must be running with the relevant models accessible.

### Interactive Demo — Adding questions

Curated questions are served from the backend `/examples` endpoint (defined in `backend/example_questions.py`). The interactive demo fetches them automatically — no frontend changes needed.

To add a new question, add an entry to `CURATED_QUESTIONS` or `TOOL_CALLING_QUESTIONS` in `example_questions.py` with the required fields (`question`, `expected_answer`, `category`, `difficulty`, `best_for`, `why`, `source`, `source_id`).
